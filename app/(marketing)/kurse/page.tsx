...
export default function CoursesPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const built: StoreItem[] = staticCourses.map((c) => {
  ...
  if (!hasMounted || loading) {
    return (
      <div className="flex min-h-[400px] items-center justify-center bg-obsidian">
        <div className="space-y-4 text-center">
          <div className="mx-auto h-10 w-10 animate-spin rounded-full border-2 border-gold-300/20 border-t-gold-300" />
          <p className="tac-label animate-pulse text-cream/70">Kurse werden geladen...</p>
        </div>
      </div>
    );
  }
  ...
  const bundleStatic = bundle ? staticCourses.find((c) => c.slug === bundle.slug) : undefined;
  ...
  return (
    <section className="relative min-h-screen overflow-hidden bg-obsidian pt-28 sm:pt-36 pb-12 sm:pb-20">
      <SpatialBackground />
      ...
      <div className="container-shell relative z-10">
        ...
        >
          ...
          <h1 className="font-heading text-4xl uppercase leading-none tracking-gta text-cream md:text-6xl">
            DIGITALE PRODUKTE <span className="gold-text">LERNEN & BAUEN.</span>
          </h1>
          <p className="mt-5 max-w-xl text-sm leading-relaxed text-cream/45">
            Video-Kurse zum Mitmachen und PDF-Guides zum Sofort-Umsetzen. Einmal kaufen,
        ...
        {/* Library — purchased (rendered if user is logged in and owns courses) */}
        {purchased.length > 0 && (
          <div className="mb-16">
