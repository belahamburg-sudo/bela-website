      ...
      <Html position={[0, 1.55, 0]} center distanceFactor={8} occlude={false}>
        <button
          ...
          className={`pointer-events-auto min-w-[154px] border px-3 py-2 text-left backdrop-blur-xl transition-all ${
            isActive
              ? "border-gold-300 bg-gold-300 text-obsidian shadow-[0_0_28px_rgba(240,180,41,0.35)]"
    ...
    <group ref={groupRef} scale={1.12}>
      <Html position={[0, 2.05, 0]} center distanceFactor={7} occlude={false}>
        <div className="pointer-events-none relative h-16 w-16">
          <div className={`absolute inset-0 rounded-[20px] border border-gold-300/40 bg-gradient-to-br ${avatar.accent} shadow-[0_16px_36px_rgba(240,180,41,0.28)]`} />
...
export function GameRoadmap({ currentPoints, avatarId }: { currentPoints: number; avatarId: string }) {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden bg-[#050403]">
      ...
      <div className="pointer-events-none absolute inset-0 z-[2] opacity-35 [background-image:linear-gradient(rgba(240,180,41,0.06)_1px,transparent_1px),linear-gradient(90deg,rgba(240,180,41,0.06)_1px,transparent_1px)] [background-size:48px_48px]" />
