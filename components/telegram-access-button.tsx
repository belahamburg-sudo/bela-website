...
import { ExternalLink, Link2, Loader2, Send } from "lucide-react";
import { paidTelegramUrl } from "@/lib/env";
...
export function TelegramAccessButton({
  ...
  const href = link.configured && link.url ? link.url : paidTelegramUrl;
  const label =
  ...
  const Icon = link.configured && link.url ? Link2 : Send;
  ...
  return (
    <div className={className}>
      ...
      {link.configured && link.linked && link.telegramUsername ? (
